...
export default function Navigation() {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-4 md:px-8 py-4 md:py-6 bg-background/50 backdrop-blur-md border-b border-white/5 transition-all">
      <div className="max-w-[1440px] mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 md:gap-6">
          <Link href="/login" className="hidden sm:block text-sm font-medium text-muted hover:text-white transition-colors">Sign In</Link>
          <Link href="/login" className="text-xs sm:text-sm font-medium bg-white text-black px-3 sm:px-4 py-2 rounded-sm hover:bg-white/90 transition-colors whitespace-nowrap">
            Start an Audit
          </Link>
        </div>
      </div>
